...
# checks for immediate wins/blocks and evaluation's scoring to order moves and
# evaluate leaf positions, with alpha-beta pruning to keep the search fast.
import random

from game.board import board, EMPTY, get_possible_moves, check_win, board_full
from game.evaluation import evaluate_move, evaluate_board_static, evaluate_candidate_move
from game.greedy_agent import find_winning_move, find_blocking_move, rank_candidate_moves
...
# Generate candidate moves ordered from most to least promising for the current player, reusing greedy's scoring, then keep only the top max_moves.
# Searching the strongest moves first lets alpha-beta pruning cut off more branches.
def get_ordered_moves(current_player, other_player, max_moves):
    possible_moves = get_possible_moves(2)
    scored_moves = rank_candidate_moves(current_player, other_player, possible_moves)

    ordered_moves = []

    for item in scored_moves:
        ordered_moves.append(item[1])
...
def minimax_search(depth, maximizing, player, opponent, alpha, beta):
    ...
    # Limit how many ordered candidate moves are explored at each node to keep the search fast.
    max_moves = 10

    if maximizing:
        best_score = -1000000000
        possible_moves = get_ordered_moves(player, opponent, max_moves)
        ...
        best_score = 1000000000
        possible_moves = get_ordered_moves(opponent, player, max_moves)
...
# Choose and make a move using Minimax search. Before searching, reuse greedy's
# tactical checks to handle an immediate win or a required block.
def minimax_algorithm(player, opponent, depth):
    ...
    # 3. Otherwise search ahead, using greedy's ranking to order candidate moves.
    possible_moves = get_ordered_moves(player, opponent, 40)
